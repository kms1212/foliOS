...
static int decode_uint(const char *str, uint64_t *value)
{
    ...
    if (str[1] == 'x') {
        for (int i = 2; str[i]; i++) {
            ...
            result <<= 4;
            result |=
                str[i] <= '9' ? str[i] - '0' : ((str[i] <= 'F' ? str[i] - 'A' : str[i] - 'a') + 10);
        }
...
)
{
    ...
    if (!total_block_count || !bytes_per_block || !blocks_per_block_group ||
        !total_block_group_count || !rbb_count) {
    ...
    for (int i = 0; i < rdb_count; i++) {
        ...
        strncpy(
            ((struct folifs_rdb *)buf)->formatted_os,
            os_name,
            sizeof(((struct folifs_rdb *)buf)->formatted_os)
        );
    ...
    for (uint64_t i = 1; i < total_block_group_count; i = (i & ~0x07) + 8) {
        ...
        ((uint8_t *)buf)[i >> 3] =
            (groups_left < 8 ? ((1 << groups_left) - 1) : 0xFF) & (0xFF << (i & 0x07));
    }
    *(uint32_t *)((uint8_t *)buf + bytes_per_block - 4) = crc32(buf, bytes_per_block - 4);
    folifs_fwrite(buf, bytes_per_block, 1, image_fp);
    fflush(image_fp);
    ...
    fseek(
        image_fp,
        bytes_per_sector * reserved_sectors + bytes_per_block * (rdb_count + rbb_count),
        SEEK_SET
    );
    memset(buf, 0, bytes_per_block);
    for (uint64_t i = rdb_count + 2 + rbb_count; i < blocks_per_block_group; i = (i & ~0x07) + 8) {
        ...
        ((uint8_t *)buf)[i >> 3] =
            (blocks_left < 8 ? ((1 << blocks_left) - 1) : 0xFF) & (0xFF << (i & 0x07));
    }
    *(uint32_t *)((uint8_t *)buf + bytes_per_block - 4) = crc32(buf, bytes_per_block - 4);
    folifs_fwrite(buf, bytes_per_block, 1, image_fp);
    fflush(image_fp);
    ...
    fseek(
        image_fp,
        bytes_per_sector * reserved_sectors + bytes_per_block * (rdb_count + 1 + rbb_count),
        SEEK_SET
    );
    memset(buf, 0, bytes_per_block);
    ...
    ((struct folifs_mdb *)buf)->attribute_entry_presence_bitmap =
        (label ? (1 << (MDB_ENTRY_FAE - 1)) : 0) | (1 << (MDB_ENTRY_TAE - 1));
    ...
    if (label) {
        ...
        ((struct folifs_mdb_fae *)((uint8_t *)buf + mdb_offset))->header.entry_length =
            sizeof(struct folifs_mdb_fae);
        ...
        ((struct folifs_mdb_fae *)((uint8_t *)buf + mdb_offset))->filename_hash =
            crc32((void *)label, label_len);
        strncpy(
            ((struct folifs_mdb_fae *)((uint8_t *)buf + mdb_offset))->filename,
            label,
            label_len
        );
    ...
    /* TAE */
    ((struct folifs_mdb_tae *)((uint8_t *)buf + mdb_offset))->header.entry_length =
        sizeof(struct folifs_mdb_tae);
    ...
    *(uint32_t *)((uint8_t *)buf + bytes_per_block - 4) = crc32(buf, bytes_per_block - 4);
    ...
    fseek(
        image_fp,
        bytes_per_sector * reserved_sectors + bytes_per_block * (rdb_count + 2 + rbb_count),
        SEEK_SET
    );
    memset(buf, 0, bytes_per_block);
    *(uint32_t *)((uint8_t *)buf + bytes_per_block - 4) = crc32(buf, bytes_per_block - 4);
