...
static void uhci_schedule_transfer(int low_speed, int endpoint, int device_addr, int packet_id, void
*buf, long len)
{
    packet_id &= 0x0F;
...
// /* initialize bus */
// uint16_t uhci_pcicfgspace_command = _bus_pci_cfg_read(uhci_device->bus, uhci_device->device,
// uhci_device->function, PCI_CFGSPACE_COMMAND, sizeof(uhci_pcicfgspace_command));
// _bus_pci_cfg_write(uhci_device->bus, uhci_device->device, uhci_device->function,
