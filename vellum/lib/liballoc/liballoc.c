...
#define ALIGN_INFO                                                                                 \
    (sizeof(ALIGN_TYPE) * 16)  ///< Alignment information is stored right before the pointer. This
...
static unsigned int l_pageSize =
    PAGE_SIZE;  ///< The size of an individual page. Set up in liballoc_init.
...
#ifdef DEBUG
    fprintf(stddbg, "liballoc: %p PREFIX(malloc)( %lu ): ", __builtin_return_address(0), size);
    FLUSH();
#endif
...
#ifdef DEBUG
    fprintf(stddbg, "liballoc: %p PREFIX(free)( %p ): ", __builtin_return_address(0), ptr);
    FLUSH();
#endif
