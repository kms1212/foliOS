...
environment. Module memory, MPK state, local heaps, interrupt entry, and driver
capabilities need a runtime contract that is narrower and more explicit than a
POSIX or C standard library ABI.
...
capabilities, and MPK-aware entry/return behavior. A separate compatibility
profile may later provide Linux-like driver helpers, but native module APIs
should remain explicit and statically checkable.
...
input when choosing or reconfiguring MPK shards. These records are planning
hints and constraints, not ambient authority: the kernel must still validate
...
the loader and MPK planner enough information to make placement and protection
decisions without parsing module code.
...
The manifest should avoid hard-coding physical MPK shard numbers. It should
describe dependencies, locality preferences, sharing requirements, and
...
porting layer; it is the place where module memory, MPK transitions, direct IRQ
upcalls, deferred interrupt replay, and driver-safe utility APIs are defined.
