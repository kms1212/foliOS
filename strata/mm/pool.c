...
static struct subpool_entry subpool_table[(
...
static uint16_t normalize_object_size(uint16_t object_size __in)
{
    uint32_t quantum_bits;
    uint32_t quantum;

    if (object_size <= (1 << STRATA_MM_POOL_MIN_ALIGN_BITS)) {
        return (uint16_t)(1 << STRATA_MM_POOL_MIN_ALIGN_BITS);
    }
    ...
    if (quantum_bits < STRATA_MM_POOL_MIN_ALIGN_BITS) {
        quantum_bits = STRATA_MM_POOL_MIN_ALIGN_BITS;
    }
...
static struct subpool_entry *get_object_entry(uint16_t normalized_object_size __in)
{
...
static int validate_subpool_header_metadata(const struct subpool_header *header)
{
    ...
    if (object_bytes != span_size - header->header_size) return 0;
...
static StStatus allocate_subpool(
    uint16_t normalized_object_size __in, struct subpool_header **header_out __out
)
{
    StStatus status;
    ...
    entry = get_object_entry(normalized_object_size);
    ...
    object_count = STRATA_MM_POOL_SUBPOOL_PAGE_COUNT * PAGE_SIZE / normalized_object_size;
    bitmap_word_count = get_subpool_bitmap_word_count(object_count);
    header_size = (uint16_t)(sizeof(*header) + (bitmap_word_count * sizeof(uint64_t)));
    object_count -= (header_size + normalized_object_size - 1) / normalized_object_size;
    bitmap_word_count = get_subpool_bitmap_word_count(object_count);
    header_size = (uint16_t)(sizeof(*header) + (bitmap_word_count * sizeof(uint64_t)));
    header_size = (uint16_t)((STRATA_MM_POOL_SUBPOOL_PAGE_COUNT * PAGE_SIZE) -
                             (normalized_object_size * object_count));
...
static St_VirtPage unlink_subpool_locked(struct subpool_header *header __in)
{
    ...
    entry = get_object_entry(header->object_size);
    if (!entry) return 0;
...
StStatus StPool_Allocate(size_t size __in, void **ptr __out)
{
    ...
    *ptr = NULL;
    ...
    normalized_size = normalize_object_size(size);
    entry = get_object_entry(normalized_size);
    if (!entry) return STATUS_TOO_LARGE;

    header = entry->next_free;
    if (!header) {
        status = allocate_subpool(normalized_size, &header);
        if (!CHECK_SUCCESS(status)) return status;
    }
...
{
    return StPool_Allocate(MAX(size, 1 << alignment), ptr);
}
...
StStatus StPool_Reallocate(void *ptr __in, size_t size __in, void **new_ptr __out)
{
    ...
    status = StPool_Allocate(size, &allocated_ptr);
    if (!CHECK_SUCCESS(status)) return status;
...
StStatus StPool_Free(void *ptr __in)
{
    ...
    subpool_header = find_subpool_header_from_ptr(ptr, &object_index);
    if (subpool_header) {
        entry = get_object_entry(subpool_header->object_size);
        if (!entry) {
            StThread_UnlockPreemption();
