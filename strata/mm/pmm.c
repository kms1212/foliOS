...
#define ALLOC_TABLE_ENTRY_INDEX_LIMIT_16M   (ALIGN_DIV(1LL << 24, PAGE_SIZE * ALLOCENT_COVERAGE_PAGES))
...
struct metadata {
    ...
    uint8_t padding[
        ...
        sizeof(struct metadata *) * 2 -
        sizeof(atomic_uint_fast32_t) * 2
    ];
} __aligned(64);

_Static_assert(
    sizeof(struct metadata) == 64,
...
static struct extended_entry early_extended_entry_pool[EARLY_EXTENDED_ENTRY_POOL_COUNT] __aligned(PAGE_SIZE);
...
static struct metadata_table early_metadata_table_pool[EARLY_METADATA_TABLE_POOL_COUNT] __aligned(PAGE_SIZE);
static int early_metadata_table_pool_used_count = 0;

static struct metadata_directory early_metadata_dir_pool[EARLY_METADATA_DIR_POOL_COUNT] __aligned(PAGE_SIZE);
...
static const uint64_t alloc_masks[63] = {
    ...
    0x0000000100000003ULL, 0x000000020000000CULL, 0x0000000400000030ULL,
    0x00000008000000C0ULL, 0x0000001000000300ULL, 0x0000002000000C00ULL,
    0x0000004000003000ULL, 0x000000800000C000ULL, 0x0000010000030000ULL,
    0x00000200000C0000ULL, 0x0000040000300000ULL, 0x0000080000C00000ULL,
    0x0000100003000000ULL, 0x000020000C000000ULL, 0x0000400030000000ULL,
    0x00008000C0000000ULL,
    
    0x000100030000000FULL, 0x0002000C000000F0ULL, 0x0004003000000F00ULL,
    0x000800C00000F000ULL, 0x00100300000F0000ULL, 0x00200C0000F00000ULL,
    0x004030000F000000ULL, 0x0080C000F0000000ULL,
    
    0x0103000F000000FFULL, 0x020C00F00000FF00ULL, 0x04300F0000FF0000ULL,
    0x08C0F000FF000000ULL,
...
{
    int offsets[] = {ALLOCENT_BMP_1P_POS,  ALLOCENT_BMP_2P_POS,
        ALLOCENT_BMP_4P_POS,  ALLOCENT_BMP_8P_POS,
        ALLOCENT_BMP_16P_POS, ALLOCENT_BMP_32P_POS};
...
static inline int find_free_frame_idx_bitmap_entry(uint64_t entry, int order)
{
    ...
    switch (order) {
        case 0: mask = ALLOCENT_BMP_1P_MASK; pos = ALLOCENT_BMP_1P_POS; break;
        case 1: mask = ALLOCENT_BMP_2P_MASK; pos = ALLOCENT_BMP_2P_POS; break;
        case 2: mask = ALLOCENT_BMP_4P_MASK; pos = ALLOCENT_BMP_4P_POS; break;
        case 3: mask = ALLOCENT_BMP_8P_MASK; pos = ALLOCENT_BMP_8P_POS; break;
        case 4: mask = ALLOCENT_BMP_16P_MASK; pos = ALLOCENT_BMP_16P_POS; break;
        case 5: mask = ALLOCENT_BMP_32P_MASK; pos = ALLOCENT_BMP_32P_POS; break;
        default: return -1;
    }
...
static inline void allocate_from_bitmap_entry(uint64_t *entry, int index, int order, int align_order)
{
    uint64_t prev = *entry;

    switch (order) {
        case 0: *entry |= alloc_masks[index]; break;
        case 1: *entry |= alloc_masks[32 + index / 2]; break;
        case 2: *entry |= alloc_masks[48 + index / 4]; break;
        case 3: *entry |= alloc_masks[56 + index / 8]; break;
        case 4: *entry |= alloc_masks[60 + index / 16]; break;
        case 5: *entry |= alloc_masks[62]; break;
    }
...
static inline void free_to_table(struct alloc_table *table, St_PhysFrame index, int order)
{
    ...
    if ((int64_t)table->entries[entry_idx].bitmap < 0) return; // Extended entry - manual handling required
    
    switch (order) {
        case 0: table->entries[entry_idx].bitmap &= ~alloc_masks[page_idx]; break;
        case 1: table->entries[entry_idx].bitmap &= ~alloc_masks[32 + page_idx / 2]; break;
        case 2: table->entries[entry_idx].bitmap &= ~alloc_masks[48 + page_idx / 4]; break;
        case 3: table->entries[entry_idx].bitmap &= ~alloc_masks[56 + page_idx / 8]; break;
        case 4: table->entries[entry_idx].bitmap &= ~alloc_masks[60 + page_idx / 16]; break;
        case 5: table->entries[entry_idx].bitmap &= ~alloc_masks[62]; break;
    }
...
static StStatus get_or_create_extentry(struct alloc_table *table, int entry_idx, struct extended_entry **entry)
{
...
static StStatus get_or_create_metadata_dir(size_t mdpa_idx, int order, struct metadata_directory **dir)
{
...
static StStatus get_or_create_metadata_table(size_t mdpa_idx, size_t metadata_dir_entry_idx, int order, struct metadata_table **table)
{
    StStatus status;
    struct metadata_directory *metadata_dir;
    ...
    if (metadata_dir->entries[metadata_dir_entry_idx]) {
        if (table) *table = (struct metadata_table *)(metadata_dir->entries[metadata_dir_entry_idx] & ~METADATA_DIR_ENTRY_LARGE_FLAG);
...
static StStatus create_metadata(St_PhysFrame pfn, void *owner, int order)
{
    ...
    if (order >= 14) {
        ...
        for (size_t i = 0; i < (1 << (order - 5)); i++) {
            metadata_dir->entries[metadata_dir_entry_idx + i] = (uintptr_t)metadata | METADATA_DIR_ENTRY_LARGE_FLAG;
        }
    } else {
        status = get_or_create_metadata_table(mdpa_idx, metadata_dir_entry_idx, order, &metadata_table);
        if (!CHECK_SUCCESS(status)) return status;
...
static StStatus get_metadata(St_PhysFrame pfn, struct metadata **metadata)
{
    ...
    if (metadata_directory->entries[entry_idx] & METADATA_DIR_ENTRY_LARGE_FLAG) {
        ...
        *metadata = (struct metadata *)(metadata_directory->entries[entry_idx] & ~METADATA_DIR_ENTRY_LARGE_FLAG);

        return STATUS_SUCCESS;
    }
    
    metadata_table = (struct metadata_table *)(metadata_directory->entries[entry_idx] & ~METADATA_DIR_ENTRY_LARGE_FLAG);
...
static void do_free_contiguous_frame(St_PhysFrame pfn, int order)
{
    ...
    if (alloc_table_ptr_array[table_idx] == ATPA_FREE || alloc_table_ptr_array[table_idx] == ATPA_UNUSABLE) {
    ...
    metadata_table = (struct metadata_table *)(metadata_dir->entries[entry_idx] & ~METADATA_DIR_ENTRY_LARGE_FLAG);
...
StStatus StPmm_MarkUsableContiguousFrame(
    St_PhysFrame base __in,
    St_PhysFrame limit __in
)
{
    ...
    while (base <= limit) {
        ...
        if (!(base % ALLOC_TABLE_COVERAGE_PAGES) && limit >= base + ALLOC_TABLE_COVERAGE_PAGES - 1) {
...
StStatus StPmm_MarkUnusableContiguousFrame(
    St_PhysFrame base __in,
    St_PhysFrame limit __in
)
{
...
)
{
    ...
    if (order >= 5) {
        ...
        for (ssize_t i = atpa_search_start; i >= 0; i -= atpa_align_jump) {
            if (alloc_table_ptr_array[i] == ATPA_FREE) {
                ...
                allocated_pfn = i * ALLOC_TABLE_COVERAGE_PAGES + table_search_start * ALLOCENT_COVERAGE_PAGES;

                // allocate and fill metadata directory & metadata
                status = create_metadata(allocated_pfn, NULL, order);
                if (!CHECK_SUCCESS(status)) return status;

                LOG_TRACE("filling allocation table entry %zd-%zd\n", table_search_start, table_search_start + table_entries_needed - 1);
                *pfn = allocated_pfn;
                free_frames -= (1ULL << order);
                return STATUS_SUCCESS;
            }
            ...
            if (
                alloc_table_ptr_array[i] == ATPA_HUGE_ALLOC ||
                alloc_table_ptr_array[i] == ATPA_UNUSABLE
            ) continue;
    ...
    for (ssize_t i = atpa_search_start; i >= 0; i -= atpa_align_jump) {
        if (alloc_table_ptr_array[i] == ATPA_FREE) {
            ...
            index = find_free_frame_idx_bitmap_entry(table->entries[table_search_start].bitmap, order);
            if (index < 0) {
            ...
            allocate_from_bitmap_entry(&table->entries[table_search_start].bitmap, index, order, align_order);

            allocated_pfn =
                i * ALLOC_TABLE_COVERAGE_PAGES +
                table_search_start * ALLOCENT_COVERAGE_PAGES +
                index;
        ...
        for (ssize_t j = table_search_start; j >= 0; j -= table_align_jump) {
            ...
            if (table->entries[j].bitmap & ALLOCENT_EXT_FLAG) {
                ...
                extentry_search_start = ((extentry_search_start / extentry_align_jump) - 1) * extentry_align_jump;
...
void StPmm_DebugDumpRegion(St_PhysFrame start_pfn, size_t count)
{
    ...
    printf("Legend: " ANSI_COLOR_GREEN "." ANSI_COLOR_RESET " Free, " 
           ANSI_COLOR_RED "*" ANSI_COLOR_RESET " Used, " 
           ANSI_COLOR_YELLOW "X" ANSI_COLOR_RESET " Unusable, "
           ANSI_COLOR_BLUE "H" ANSI_COLOR_RESET " HugeAlloc\n\n");
    ...
    while (current_pfn < end_pfn) {
        ...
        if (entry->bitmap == ALLOCENT_EXT_UNUSABLE) {
            ...
            switch (flag) {
                case EE_FREE:     symbol = ANSI_COLOR_GREEN "." ANSI_COLOR_RESET; break;
                case EE_USED:     symbol = ANSI_COLOR_RED "*" ANSI_COLOR_RESET; break;
                case EE_UNUSABLE: symbol = ANSI_COLOR_YELLOW "X" ANSI_COLOR_RESET; break;
...
void StPmm_DebugDumpAtpa(void)
{
    ...
    for (int i = 0; i < ALLOC_TABLE_PTR_ARRAY_SIZE; i++) {
        ...
        printf("[%04d: ", i);
        if (val == ATPA_FREE) printf(ANSI_COLOR_GREEN "FREE" ANSI_COLOR_RESET);
        else if (val == ATPA_HUGE_ALLOC) printf(ANSI_COLOR_BLUE "HUGE" ANSI_COLOR_RESET);
        else printf("TBL"); // Table exists (Partial/Full)
        printf("] ");
