...
#define SET_INT_ENTRY(num)                                                                         \
    ...
    {                                                                                              \
        .offset_low = (uint32_t)_pc_isr_##num & 0xFFFF, .segment_selector = SEG_SEL_KERNEL_CODE,   \
        .attributes.raw = 0x8E, .offset_high = ((uint32_t)_pc_isr_##num >> 16) & 0xFFFF,           \
    }
...
#define SET_TRAP_ENTRY(num)                                                                        \
    ...
    {                                                                                              \
        .offset_low = (uint32_t)_pc_isr_##num & 0xFFFF, .segment_selector = SEG_SEL_KERNEL_CODE,   \
        .attributes.raw = 0x8F, .offset_high = ((uint32_t)_pc_isr_##num >> 16) & 0xFFFF,           \
    }

    struct StA_IdtEntry _pc_idt[256] __aligned(PAGE_SIZE);
struct StInt_Handler *_pc_isr_table[256];
        ...
        if (is_fault) {
            if (has_error) {
                St_Panic(
                    ...
                    "Unrecoverable fault #%02X(0x%08X) has occurred at 0x%04X:0x%08" PRIX32 "",
                    num,
                    frame->error,
                    frame->cs,
                    frame->eip
                ...
                St_Panic(
                    STATUS_UNKNOWN_ERROR,
                    "Unrecoverable fault #%02X has occurred at 0x%04X:0x%08" PRIX32 "",
                    num,
                    frame->cs,
                    frame->eip
                );
