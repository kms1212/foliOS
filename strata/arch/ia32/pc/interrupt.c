...
void *_pc_isr_common(struct StA_InterruptFrame *frame, struct StIntP_Context *ctx, int num)
{
    ...
    if (!current_isr) {
        if (is_fault) {
            if (has_error) {
                St_Panic(STATUS_UNKNOWN_ERROR, "Unrecoverable fault #%02X(0x%08X) has occurred at 0x%04X:0x%08"PRIX32"", num, frame->error, frame->cs, frame->eip);
            } else {
                St_Panic(STATUS_UNKNOWN_ERROR, "Unrecoverable fault #%02X has occurred at 0x%04X:0x%08"PRIX32"", num, frame->cs, frame->eip);
            }
        } else {
            ILOG_WARN("unhandled interrupt #%02X at 0x%04X:0x%08"PRIX32"\n", num, frame->cs, frame->eip);
        }
    }
