...
void test_extended_entry_allocation(void)
{
    ...
    if (allocations_in_hole == 0) {
        TEST_LOG("[WARN] Did not hit the extended entry hole. Allocator logic might have preferred other areas.");
    }
}
...
void test_pmm_fuzzing(void)
{
    ...
    for (int i = 0; i < FUZZ_ITERATIONS; i++) {
        ...
        if (action == 0) {
            ...
            size_t count;
            int r = rand() % 100;
            if (r < 70) count = 1 + (rand() % 4);       // 70% : 1~4 frames (Small)
            else if (r < 95) count = 5 + (rand() % 60); // 25% : 5~64 frames (Medium)
            else count = 16384;                         // 5%  : 64MiB (Huge)
            ...
            if (CHECK_SUCCESS(s)) {
                // TEST_LOG("allocated to pfn %lX (%zu frames, align 2^%d)", pfn, count, align_pow2);
