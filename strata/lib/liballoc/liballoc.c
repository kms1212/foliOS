...
static struct liballoc_major *allocate_new_page( unsigned int size )
{
	...
	struct liballoc_major *maj;
		...
		#ifdef DEBUG
		LOG_DEBUG( "liballoc: Resource allocated %p of %u pages (%u bytes) for %u size.\n", (void *)maj, st, maj->size, size );
...
void *PREFIX(malloc)(size_t req_size)
{
	...
	#ifdef DEBUG
	LOG_DEBUG( "liballoc: %p PREFIX(malloc)( %lu ): ", 
					__builtin_return_address(0),
					size );
	FLUSH();
	#endif
...
void PREFIX(free)(void *ptr)
{
	...
	if ( ptr == NULL ) 
	{
		l_warningCount += 1;
		#if defined DEBUG || defined INFO
		LOG_DEBUG( "liballoc: WARNING: PREFIX(free)( NULL ) called from %p\n",
							__builtin_return_address(0) );
		FLUSH();
		#endif
		return;
	...
	if ( min->magic != LIBALLOC_MAGIC ) 
	{
		...
		{
			#if defined DEBUG || defined INFO
			LOG_DEBUG( "liballoc: ERROR: multiple PREFIX(free)() attempt on %p from %p.\n", 
									ptr,
									__builtin_return_address(0) );
			FLUSH();
			#endif
		...
		{
			#if defined DEBUG || defined INFO
			LOG_DEBUG( "liballoc: ERROR: Bad PREFIX(free)( %p ) called from %p\n",
								ptr,
								__builtin_return_address(0) );
			FLUSH();
			#endif
	...
	#ifdef DEBUG
	LOG_DEBUG( "liballoc: %p PREFIX(free)( %p ): ", 
				__builtin_return_address( 0 ),
				ptr );
	FLUSH();
	#endif
