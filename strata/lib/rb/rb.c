...
static int check_order(struct StRbtree *rbt, struct StRbtree_Node *n, void *min, void *max); /* Note: check_order verification logic might need adjustment or be disabled if we can't easily track min/max without void* */
static int check_black_height(struct StRbtree *rbt, struct StRbtree_Node *node);
static void print(struct StRbtree *rbt, struct StRbtree_Node *node, StRbtree_PrintFunc print_func, int depth, char *label);
...
struct StRbtree_Node *StRbtree_Min(struct StRbtree *rbt)
{
    ...
    while (p->left != RB_NIL(rbt)) p = p->left;
    return p;
}
...
struct StRbtree_Node *StRbtree_Max(struct StRbtree *rbt)
{
    struct StRbtree_Node *p = RB_FIRST(rbt);
    if (p == RB_NIL(rbt)) return NULL;
    while (p->right != RB_NIL(rbt)) p = p->right;
    return p;
}
 ...
 */
int rb_apply(struct StRbtree *rbt, struct StRbtree_Node *node, StRbtree_ApplyFunc func, void *cookie, enum StRbtree_TraversalType order)
{
	int err;
...
void StRbtree_Insert(struct StRbtree *rbt, struct StRbtree_Node *node)
{
	...
	while (current != RB_NIL(rbt)) {
		...
		if (cmp == 0) {
             ...
             * Users should check StRbtree_Find before Insert if they want to avoid duplicates/update.
             * For now, we'll assume duplicate keys are allowed or handled by caller.
             * If not allowed, we could return early, but the signature returns void.
             * Let's follow standard intrusive behavior: if dup, usually we just insert it (multiset) or undefined.
             * But if RB_DUP is NOT defined, we should probably not insert?
             * The original code updated the data. Here we can't.
             * Let's just assume we insert it to the right or left.
             * Actually, consistent with original: if equal, update. 
             * But we can't update data. We could replace the node in the tree?
             * That's complex. Let's just proceed as if cmp != 0 if we can't support update.
             * Or better, just follow cmp < 0 ? left : right.
             */
	...
	#ifdef RB_MIN
	if (rbt->min == NULL || rbt->compare(current, rbt->min) < 0)
		rbt->min = current;
	#endif
...
void StRbtree_Delete(struct StRbtree *rbt, struct StRbtree_Node *node)
{
	...
	if (node->left == RB_NIL(rbt) || node->right == RB_NIL(rbt)) {
        ...
        /* 
         * Intrusive swap: we cannot swap data. We must swap nodes physically in the tree architecture.
         * The original code swapped data (`node->data = target->data`) and deleted `target`.
         * Since we can't swap data, we must strictly splice out `target`, and if `target != node`,
         * we must replace `node` with `target` in the tree structure (updates colors, parent/child links). 
         */
    ...
    if (target != node) {
        ...
        if (node->parent == RB_ROOT(rbt)) {
             ...
             */
             ...
             // But wait, what if node is right child of sentinel? (Shouldn't happen for root logic usually used here).
 ...
 */
void print(struct StRbtree *rbt, struct StRbtree_Node *n, StRbtree_PrintFunc print_func, int depth, char *label)
{
	if (n != RB_NIL(rbt)) {
		...
		if (label)
			printf("%s: ", label);
		if (print_func) print_func(n);
        else printf("%p", (void*)n);
		printf(" (%s)\n", n->color == RED ? "r" : "b");
